<template>
    <div>
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            ...
            <a @click.prevent="showForm()" href="#" class="btn btn-primary"><i
                    class="far fa-plus-square fa-fw fa-sm mr-2"></i>Adicionar produto</a>
        </div>
        ...
        <div class="input-group form-group">
            ...
            <div class="input-group-append">
                <button @click="clearSearch" v-if="isSearching" class="btn btn-danger"><i
        ...
        <div v-if="produtos.length > 0">
            <article v-for="(produto, index) in produtos" :key="produto.id"
                class="position-relative card card-body shadow-sm mb-4 border-0">
                <div class="actions">
                    <div class="dropdown">
                        <button href="#" data-toggle="dropdown"><i class="fas fa-bars fa-fw"></i></button>
                        <div class="dropdown-menu dropdown-menu-right shadow-lg">
                            <a href="#" @click.prevent="showForm(index)" class="dropdown-item"><i
                                    class="far fa-edit fa-fw fa-sm mr-2"></i>Alterar</a>
                            <a href="#" @click.prevent="remove(index)" class="dropdown-item"><i
                                    class="far fa-trash-alt fa-fw fa-sm mr-2"></i>Remover</a>
                        </div>
                    </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input v-model="form.codigo" type="text" name="codigo" id="codigo" class="form-control"
                                autocomplete="off">
                            <span class="form-text text-muted">O código deve ser único.</span>
                            <strong class="text-danger py-1 d-block" v-if="form.errors.has('codigo')"
                                v-text="form.errors.get('codigo')"></strong>
                        </div>
                        <div class="form-group">
                            <label for="titulo">Título *</label>
                            <input v-model="form.titulo" type="text" name="titulo" id="titulo" class="form-control"
                                autocomplete="off">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input v-model="form.valor" type="number" name="valor" id="valor" class="form-control"
                                    autocomplete="off">
                            </div>
                            <strong class="text-danger py-1 d-block" v-if="form.errors.has('valor')"
                                v-text="form.errors.get('valor')"></strong>
                        </div>

                        <div class="form-group">
                            <label for="observacao">Observações</label>
                            <textarea v-model="form.observacao" name="observacao" id="observacao" cols="30" rows="5"
                    ...
                    <div class="modal-footer">
                        <button @click="isEditing !== false ? update(isEditing) : create()" type="button"
                            class="btn btn-success">{{ isEditing !== false ? 'Editar' : 'Adicionar' }}</button>
                    </div>
                </div>
            </div>
        </div>
